...
def main():
    ...
    p.add_argument('--representation-policy', choices=('coarse','dense_fine'), required=True,
                   help='Coupled count/size/motion/initial-area recipe; isolated experimental entrypoint')
    ...
    recipe = representation_recipe(a.representation_policy)
    validate_representation_scope(native_rgb=a.native_rgb_training,
        shape_cohort=a.candidate_shape_cohort, kernel=a.leaf_optical_kernel)
    if a.depth_search_policy != 'photometric_volume':
        raise ValueError('Representation pair requires the same photometric-volume search')
    a.rays_per_view = recipe.rays_per_view
    a.position_radius_sigmas = recipe.position_radius_sigmas
    ...
    if not (1 <= a.rays_per_view <= 4096 and a.steps > 0 and a.eval_every > 0 and 1 <= a.image_cache_size <= 512
    ...
    seed_initial_opacities = []; representation_seed_audit = []
            ...
            if len(y):
                ...
                if a.depth_search_policy != 'fixed_layers':
                    ...
                    proposals = stratified_ray_candidates(seed_camera(v), uv, selected_depth,
                        v.original_image.cuda()[:, y, x].T, seed=37001+index, pixel_sigma=recipe.pixel_sigma)
                    if a.depth_search_policy == 'photometric_volume':
                    ...
                    seeds.append(ray_candidates(seed_camera(v), torch.stack((x, y), 1).float(), selected_depth,
                                                v.original_image.cuda()[:, y, x].T, a.ratios, pixel_sigma=recipe.pixel_sigma))
                peak = recipe.initial_peak_opacity(ray_population)
                seed_initial_opacities.append(torch.full((len(y)*3,), peak, device='cuda', dtype=depth.dtype))
                representation_seed_audit.append(dict(index=index, population=ray_population,
                    rays=len(y), pixel_sigma=recipe.pixel_sigma, initial_peak_opacity=peak,
                    nominal_projected_area_ratio=recipe.nominal_projected_area_ratio(ray_population)))
    ...
    if len(base)+len(xyz) > 4000000: raise ValueError('Experimental representation capacity exceeded')
    candidate = CandidateCloud(xyz, scales, colors)
    with torch.no_grad():
        initial_peaks = torch.cat(seed_initial_opacities).to(candidate.logits)
        if initial_peaks.shape != candidate.logits.shape or not ((initial_peaks >= .001) & (initial_peaks <= .004)).all():
            raise ValueError('Aligned low-opacity per-seed initialization required')
        candidate.logits.copy_(torch.logit(initial_peaks))
    ...
    manifest = dict(scope=('joint_persistent_optics_and_unverified_candidates__rigid_frozen__optional_bounded_source_positions'
                    ...
                    candidate_count=len(candidate.xyz),
                    initial_opacity=dict(policy='per_seed_nominal_projected_area_budget',
                                         minimum=float(initial_peaks.min()), maximum=float(initial_peaks.max())),
                    representation_seed_audit=representation_seed_audit,
                    representation_helper_sha256=sha256_file(Path(__file__).with_name('canopy_representation_resolution.py')),
