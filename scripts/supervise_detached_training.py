...
def _establish_contract(config: SupervisorConfig) -> None:
...
def _supervise(
    ...
    *,
    ...
    current_command = config.base_command
    ...
    while True:
        ...
        if baseline_checkpoint is not None and not _resume_positions(
            current_command, config.resume_flag
        ):
            raise SupervisorError(
                "Configured checkpoint already exists, but the launch command "
                f"does not contain {config.resume_flag}; refusing to overwrite "
                "a resumable run"
            )
        if baseline_checkpoint is None and _resume_positions(
            current_command, config.resume_flag
        ):
            raise SupervisorError(
                f"Launch command requests {config.resume_flag}, but configured "
                f"checkpoint is absent: {config.checkpoint}"
            )
