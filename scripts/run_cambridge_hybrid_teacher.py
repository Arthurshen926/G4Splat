...
PROFILES["moge3_static_quality"] = {
    **PROFILES["quality"],
    ...
    "maximum_open_shared_envelope_localization_rows": 2_048,
...
def _teacher_result_is_current(
    ...
    *,
    ...
    maximum_shared_envelope_localizations_per_event: int = 0,
    maximum_open_shared_envelope_localization_rows: int = 2_048,
    shared_envelope_localization_every: int = 500,
    ...
    try:
        ...
        return bool(
            ...
            and (
                int(maximum_shared_envelope_localizations_per_event) == 0
                or (
                    int(
                        contract.get(
                            "shared_envelope_ownership_localization", {}
                        ).get("maximum_groups_per_event", -1)
                    )
                    == int(maximum_shared_envelope_localizations_per_event)
                    and int(
                        contract.get(
                            "shared_envelope_ownership_localization", {}
                        ).get("maximum_open_transaction_rows", -1)
                    )
                    == int(maximum_open_shared_envelope_localization_rows)
                    and int(
                        contract.get(
                            "shared_envelope_ownership_localization", {}
                        ).get("every", -1)
                    )
                    == int(shared_envelope_localization_every)
                    and int(
                        contract.get(
                            "shared_envelope_ownership_localization", {}
                        ).get("start_iteration_exclusive", -1)
                    )
                    == int(shared_envelope_localization_start_iteration)
                    and int(
                        contract.get(
                            "shared_envelope_ownership_localization", {}
                        ).get("until_iteration_inclusive", -1)
                    )
                    == int(shared_envelope_localization_until_iteration)
                    and np.isclose(
                        float(
                            contract.get(
                                "shared_envelope_ownership_localization", {}
                            ).get("receiver_optical_mass_fraction", np.nan)
                        ),
...
def _teacher_command(
    *,
    ...
    if int(maximum_shared_envelope_localizations_per_event) > 0:
        if shared_envelope_localization_start_iteration is None or (
            shared_envelope_localization_until_iteration is None
        ):
            raise ValueError(
                "Enabled shared-envelope localization requires explicit "
                "start and end iterations"
            )
        command.extend(
            [
                "--maximum-shared-envelope-localizations-per-event",
                str(int(maximum_shared_envelope_localizations_per_event)),
                "--maximum-open-shared-envelope-localization-rows",
                str(int(maximum_open_shared_envelope_localization_rows)),
                "--shared-envelope-localization-every",
                str(int(shared_envelope_localization_every)),
                "--shared-envelope-localization-start-iteration",
                str(int(shared_envelope_localization_start_iteration)),
                "--shared-envelope-localization-until-iteration",
                str(int(shared_envelope_localization_until_iteration)),
                "--static-detail-materialization-mass-fraction",
                str(float(static_detail_materialization_mass_fraction)),
            ]
        )
...
def main() -> None:
    ...
    if "train_teacher" in selected:
        ...
        result_current = _teacher_result_is_current(
            ...
            maximum_shared_envelope_localizations_per_event=int(
                profile.get("maximum_shared_envelope_localizations_per_event", 0)
            ),
            maximum_open_shared_envelope_localization_rows=int(
                profile.get("maximum_open_shared_envelope_localization_rows", 2_048)
            ),
            shared_envelope_localization_every=int(
                profile.get("shared_envelope_localization_every", 500)
            ),
            shared_envelope_localization_start_iteration=profile.get(
                "shared_envelope_localization_start_iteration"
            ),
            shared_envelope_localization_until_iteration=profile.get(
                "shared_envelope_localization_until_iteration"
            ),
        ...
        if not result_current:
            command = _teacher_command(
                ...
                maximum_shared_envelope_localizations_per_event=int(
                    profile.get(
                        "maximum_shared_envelope_localizations_per_event", 0
                    )
                ),
                maximum_open_shared_envelope_localization_rows=int(
                    profile.get(
