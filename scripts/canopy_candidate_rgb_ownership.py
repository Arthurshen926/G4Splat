...
def candidate_rgb_loss(rgb, target, reference, regions, *, noncanopy_target):
    if noncanopy_target not in ('source_reference', 'legacy_ground_truth', 'observed_risk'):
    ...
    if noncanopy_target in ('source_reference','observed_risk') and (reference is None or reference.shape != rgb.shape):
        raise ValueError('Immutable original scene RGB required outside canopy')
    tree_error = (rgb-target).abs().mean(0)
    if noncanopy_target=='observed_risk':
        from scripts.canopy_observed_background_risk import observed_rgb_risk
        return ((tree_error*regions['tree']).sum()/regions['tree'].sum().clamp_min(1)
                +sum(observed_rgb_risk(rgb,target,reference,regions[key]) for key in ('rigid','sky')))
