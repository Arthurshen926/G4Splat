...
// Experimental projected optical-depth kernel. Surface kernels stay unchanged.
// Keep the original footprint and depth ordering for a controlled comparison.
__device__ __forceinline__ float canopyTau(float peak)
{
	return -log1pf(-min(1.0f - 1.0e-6f, max(0.0f, peak)));
}

__device__ __forceinline__ float canopyAlpha(float peak, float G)
{
	return -expm1f(-canopyTau(peak) * G);
}

__device__ __forceinline__ float canopyAlphaDerivativeG(float peak, float G)
{
	const float tau = canopyTau(peak);
	return canopyAlpha(peak, G) >= 0.99f ? 0.0f : tau * expf(-tau * G);
}

__device__ __forceinline__ float canopyAlphaDerivativePeak(float peak, float G)
{
	if (peak <= 0.0f || peak >= 1.0f - 1.0e-6f || canopyAlpha(peak, G) >= 0.99f)
		return 0.0f;
	return G * expf(-canopyTau(peak) * G) / (1.0f - peak);
}
	...
	int gate_size)
{
	...
	value.alpha = min(
		0.99f,
		value.is_surface ? primitive.shape.w * value.G * value.gate_value
			: canopyAlpha(primitive.shape.w, value.G));
