	...
	float* __restrict__ dL_dcolors)
{
			...
			|| dL_dvolume_hit_interval_alpha != 0.0f))
	{
		for (uint32_t offset = range.x; offset < range.y; ++offset)
		{
			...
			if (canopyAlpha(shape.w, G) >= 0.99f) continue;
			grad /= max(1.0f - candidate.alpha, 1.0e-6f);
			const float gradG = canopyAlphaDerivativeG(shape.w, G) * grad;
			...
			atomicAdd(&dL_dopacity[id], canopyAlphaDerivativePeak(shape.w, G) * grad);
		}
	}
	...
	while (!done)
	{
		...
		if (!best.valid || best.id != item.id || best.depth != item.depth)
			...
			const float dL_dG = (is_surface ? shape.w * gate_value
				: canopyAlphaDerivativeG(shape.w, G)) * dL_dalpha;
			...
			atomicAdd(
				&dL_dopacity[id], (is_surface ? G * gate_value
					: canopyAlphaDerivativePeak(shape.w, G)) * dL_dalpha);
			if (gate_index >= 0)
			{
