...
class ParallelAligner(torch.nn.Module):
    ...
    def _verts_deformations_for_uv(
        ...
        *,
        ...
        """Evaluate the deformation network for a contiguous Chart pixel span.

        ``pts_uv`` may cover the full Chart grid or only a contiguous range of
        rows.  Keeping the point span explicit lets the final materialisation
        stream MLP activations instead of constructing every Chart's output at
        once.
        """
        ...
        # Weight encodings with confidence if needed.  Evaluate just the
        # requested confidence rows so the chunked export remains low-memory.
        ...
        # If we are predicting deformations along rays, multiply by the
        # corresponding slice rather than the full n_charts * H * W tensor.
    ...
    def materialize_deformed_verts(self, *, chunk_rows: int = 64) -> None:
        """Write final vertices in spatial chunks to avoid a terminal OOM.

        The alignment objective remains dense.  This only changes the final
        no-grad export from the learned deformation field into
        ``_deformed_verts``.
        """
        ...
        if self.device.type == "cuda":
            # Release stale optimizer allocations before the terminal forward;
            # the chunks below then cap the live MLP activation footprint.
    ...
    def optimize(
        ...
        # A full-resolution final deformation forward can be substantially
        # larger than any individual optimization step.  Stream it by rows;
        # use the configured regularizer chunk when present and a conservative
        # default otherwise.
        final_chunk_rows = (
            norm_chunk_rows
            if int(chart_encoding_norm_chunk_rows) > 0
            else min(64, self.pm_h)
        )
        self.materialize_deformed_verts(chunk_rows=final_chunk_rows)
        progress_bar.close()
        if verbose:
