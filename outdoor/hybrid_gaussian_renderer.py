...
class VolumetricFoliageModel(nn.Module):
    ...
    def materialize_static_detail_receivers(
        ...
        *,
        ...
        parent_retired_fraction = self.handoff_retired_fraction[
            parent_rows
        ].detach().clone().clamp(0.0, 0.95)
        parent_reference_mass = self.handoff_reference_mass[
            parent_rows
        ].detach().clone()
        # A legacy/uninitialized reference is reconstructed from the exact
        # current mass and retired fraction.  For an initialized late-stage
        # envelope this leaves the persisted reversible history untouched.
        parent_reference_mass = torch.where(
            parent_reference_mass > 0,
            parent_reference_mass,
            parent_mass / (1.0 - parent_retired_fraction).clamp_min(1.0e-4),
        )
        parameter_values = {}
        for name in (
            "xyz",
        ...
        # The child is a new detail receiver and owns no prior envelope
        # replacement history.  The late-stage parent, however, must retain
        # its accumulated overlap/camera evidence and retired fraction.  The
        # old implementation cleared both rows, silently erasing reversible
        # ownership history whenever receiver factorization happened late.
        for name in (
            "replacement_overlap_ema",
            "replacement_observation_count",
            "replacement_camera_signature",
        ...
        metadata["handoff_reference_mass"][parent_rows] = (
            (1.0 - fraction) * parent_reference_mass
        )
        ...
        return {
            ...
            "late_handoff_history_preserved": True,
            "_new_to_old": new_to_old,
            "_new_start": old_count,
            "_parent_rows": parent_rows,
        }
        ...
        *,
        ...
        for name in (
            ...
            if name == "xyz":
                ...
                child_optical_depth = child_tau.squeeze(-1)
            parameter_values[name] = torch.cat([value[keep], child], dim=0)
        camera_plane_mass_before = 0.0
        camera_plane_mass_after = 0.0
        if bool(optical_plane_parent.any()):
            parent_first_axis = axis_order[:, 0]
            parent_second_axis = axis_order[:, 1]
            parent_plane_area_unique = (
                scales.gather(1, parent_first_axis[:, None])[:, 0]
                * scales.gather(1, parent_second_axis[:, None])[:, 0]
            )
            parent_tau_unique = -torch.log1p(
                -self.opacities.detach()[indices].clamp(
                    1.0e-6, 1.0 - 1.0e-6
                )
            )
            camera_plane_mass_before = float(
                (
                    parent_tau_unique[optical_plane_parent]
                    * parent_plane_area_unique[optical_plane_parent]
                ).sum()
            )
            if child_optical_depth is None:
                raise RuntimeError(
                    "Camera-plane split did not construct child optical depth"
                )
            camera_plane_mass_after = float(
                (
                    child_optical_depth[optical_plane_child]
                    * child_plane_area[optical_plane_child]
                ).sum()
            )
            plane_tolerance = max(
                abs(camera_plane_mass_before), 1.0e-8
            ) * 5.0e-6
            if (
                abs(camera_plane_mass_after - camera_plane_mass_before)
                > plane_tolerance
            ):
                raise RuntimeError(
                    "Camera-plane split changed owner-plane optical mass"
                )
        metadata = {
            name: torch.cat(
                [
        ...
        return {
            ...
            "camera_plane_optical_mass_before": camera_plane_mass_before,
            "camera_plane_optical_mass_after": camera_plane_mass_after,
            "pruned": int(remove.sum()),
            "_new_to_old": new_to_old,
